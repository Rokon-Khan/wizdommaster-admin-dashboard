...
import { Home, Grid3x3, Settings, Bell, Heart, ChevronRight } from "lucide-react";
        ...
        <aside className="fixed left-0 top-0 h-full bg-white border-r border-gray-200 w-16 md:w-20 z-40 shadow-sm hidden md:flex flex-col">
            <div className="flex flex-col h-full py-4 md:py-6">
                ...
                <div className="px-2 md:px-4 mb-6 md:mb-8">
                    ...
                    <span className="text-gray-900 font-semibold text-xs md:text-sm hidden lg:block">Wellmetrix</span>
                </div>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                title={item.label}
                            >
                                <Icon className={`w-4 h-4 md:w-5 md:h-5 ${isActive ? 'fill-gray-900' : ''}`} />
                            </Link>
                        );
                    })}
                </nav>
